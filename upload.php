...
class MogileFS
{
	...
	public function fileinfo($key)
	{
		...
		for($i=0;$i<$this->retrycount+1;$i++) {
			try {
				$result = $this->doRequest(MogileFS::FILE_INFO, array('key' => $key));
				fclose($this->socket); // In case tracker failed, try another if possible
				return $result;
			} catch (Exception $e) {
				if ($i>=$this->retrycount) {
...
// Modified by Thanh Son (thanhson1085@gmail.com)
if (1)
  {
  if ($_FILES["file"]["error"] > 0)
    ...
    $mfs = new MogileFS('testdomain', 'testclass', '127.0.0.1:7001');
    $mfs->setFile('test123',  $_FILES["file"]["tmp_name"]);
    printf("Upload is OK(0/1)?: %d\n", $mfs->exists('test123'));
    }
  }
else
  {
